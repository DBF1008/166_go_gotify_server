...
func bootTestServer(handlerFunc gin.HandlerFunc) (*httptest.Server, *API) {
	return bootTestServerWithHistory(handlerFunc, nil)
}

func bootTestServerWithHistory(handlerFunc gin.HandlerFunc, history MessageHistory) (*httptest.Server, *API) {
	r := gin.New()
	r.Use(handlerFunc)
	// ping every 500 ms, and the client has 500 ms to respond
	api := New(500*time.Millisecond, 500*time.Millisecond, []string{}, history)
